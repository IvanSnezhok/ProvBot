...
from states.get_client import BroadcastStates
...
async def stats(message: types.Message):
    await db.message(
        message.from_user.full_name, message.from_user.id, message.text, message.date
    )
    await message.answer(
        "Кількість користувачів: "
        + str(await db.count_users())
        + "\n"
        + "Кількість користувачів с договором: "
        + str(await db.count_contract_users()),
        reply_markup=back,
    )
...
async def admin_panel(message: types.Message, state: FSMContext):
    ...
    if result:
        if type(result) is dict:
            await message.answer(
                text=await format_text_account_admin(result),
                reply_markup=admin_account_menu,
            )
        ...
        await db.message(
            message.from_user.full_name, message.from_user.id, msg.html_text, msg.date
        )
...
async def admin_panel(call: types.CallbackQuery, state: FSMContext):
    ...
    await db.message(
        call.from_user.full_name, call.from_user.id, msg.html_text, msg.date
    )
...
async def admin_answer(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"Напишіть текст або надішліть фото/документ/відео"
        f" для відправлення користувачу {user_id}",
        reply_markup=back,
    )
...
async def message_history_start(call: types.CallbackQuery, state: FSMContext):
    ...
    msg = await call.message.answer(
        text="Напишіть кількість останніх повідомлень що бажаєте передивитися\n"
...
async def admin_answer_text(message: types.Message, state: FSMContext):
    ...
    if message.content_type == "text":
        ...
        for admin in ADMINS:
            msg = await dp.bot.send_document(
                admin,
...
async def admin_change_balance(call: types.CallbackQuery, state: FSMContext):
    ...
    msg = await call.message.answer(
        "Введіть сумму на яку хочете змінити баланс\n"
...
async def account_menu(call: types.CallbackQuery, state: FSMContext):
    ...
    msg = await call.message.answer(
        "Виберіть як будемо шукати абонента", reply_markup=search_choice
    )
...
@dp.callback_query_handler(
    IDFilter(ADMINS),
    text=["search_contract", "search_phone", "search_name", "search_address"],
    state="account_menu",
)
async def search_account(call: types.CallbackQuery, state: FSMContext):
    ...
    if call.data == "search_contract":
        ...
        msg = await call.message.answer(
            "Введіть вулицю, будинок та квартиру через пробіл"
        )
        await db.message("BOT", 10001, msg.html_text, msg.date)
...
@dp.message_handler(
    IDFilter(ADMINS),
    state=["search_contract", "search_phone", "search_name", "search_address"],
)
...
async def account_menu_list(call: types.CallbackQuery, state: FSMContext):
    ...
    msg = await call.message.answer(
        text=await format_text_account_admin(result), reply_markup=admin_account_menu
    )
    await state.set_data({"account": result["contract"]})
    await db.message("BOT", 10001, msg.html_text, msg.date)
...
async def message_history_get(message: types.Message, state: FSMContext):
    ...
    messages = await db.get_message_history(
        user_id=user_id[0][0], message_count=int(message.text)
    )
    msg = await message.answer(
    ...
    for i in messages:
        if i[0] is not None:
            ...
            msg2 = await message.answer(
                "Пусте повідомлення (натискання на кнопку або відправка файлів)"
            )
...
async def admin_temporary_payment(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if await t_pay(account):
            msg = await call.message.answer(
                text="Тимчасовий платіж увімкнений успішно для абонента " + account
            )
        else:
            msg = await call.message.answer(
                text="Тимчасовий платіж не вдалося ввімкнути для абонента " + account
            )
    except KeyError:
        contract = re.search(r"\b\d{8}\b", call.message.text)
        if await t_pay(contract[0]):
            msg = await call.message.answer(
                text="Тимчасовий платіж увімкнений успішно для абонента " + contract[0]
            )
        else:
            msg = await call.message.answer(
                text="Тимчасовий платіж не вдалося ввімкнути для абонента "
                + contract[0]
            )
    result = await find(contract=call.data[17:])
    msg1 = await call.message.answer(
        text=await format_text_account_admin(result), reply_markup=admin_account_menu
    )
    await state.set_data({"account": result["contract"]})
...
async def admin_change_balance_handler(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        msg = await message.answer(
            text=f"Баланс користувача {account} поповнено на {message.text}",
            reply_markup=back,
        )
...
@dp.callback_query_handler(
    IDFilter(ADMINS), lambda call: call.data == "panel_send_message"
)
async def send_message(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.answer()
    msg = await dp.bot.send_message(
        call.from_user.id, text, reply_markup=ReplyKeyboardRemove()
    )
...
async def message_get_phone(message: types.Message, state: FSMContext):
    ...
    msg = await message.answer(
        "Напишіть текст повідомлення, або відправте фотографію/документ",
...
async def message_get_text(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if int(data["phone"]) in users_id:
            msg = await message.answer(
                "ІД знайдений у боті, відправляти?", reply_markup=accept_message
            )
            await db.message(
                message.from_user.full_name,
                message.from_user.id,
            ...
            msg = await message.answer(
                "Телефон знайдений у боті, відправляти?",
                reply_markup=accept_message_phone,
            )
            await db.message(
                message.from_user.full_name,
                message.from_user.id,
            ...
            msg = await message.answer(
                "Телефон або ІД не знайдений у боті, відправити смс?",
...
@dp.callback_query_handler(
    IDFilter(ADMINS),
    lambda call: call.data == "panel_send_message_decline",
    state="send_message_text",
)
@dp.callback_query_handler(
    IDFilter(ADMINS),
    lambda call: call.data == "panel_send_message_accept",
    state="send_message_text",
)
async def message_send_accept(call: types.CallbackQuery, state: FSMContext):
    ...
    if type == "text":
        ...
        if call.data == "panel_send_message_accept":
            try:
                msg_u = await dp.bot.send_message(
                    chat_id=int(telegram_id), text=text, parse_mode="HTML"
                )
            ...
            msg = await dp.bot.send_message(
                call.from_user.id, "Повідомлення не відправлено", reply_markup=back
            )
            await db.message("BOT", 10001, msg.html_text, msg.date)
            await state.finish()
    elif type == "photo":
        ...
        if call.data == "panel_send_message_accept":
            try:
                ...
                for admin in ADMINS:
                    ...
                    photo_send = await dp.bot.send_photo(
                        admin,
                        photo=photo,
                        caption="Фото відправлено",
                        reply_markup=back,
                    )
                    await db.message("BOT", 10001, msg.html_text, msg.date)
                    await db.message(
                        "BOT", 10001, photo_send.photo[-1].file_id, photo_send.date
                    )
                await state.finish()
            except Exception as e:
                msg = await dp.bot.send_message(
        ...
        if call.data == "panel_send_message_accept":
            try:
                msg_u = await dp.bot.send_document(
                    chat_id=int(telegram_id), document=document
                )
                ...
                for admin in ADMINS:
                    ...
                    doc_send = await dp.bot.send_document(
                        admin,
                        document=document,
                        caption="Документ відправлено",
                        reply_markup=back,
                    )
                    await db.message("BOT", 10001, msg.html_text, msg.date)
                    await db.message(
                        "BOT", 10001, doc_send.document.file_id, doc_send.date
                    )
                await state.finish()
            except Exception as e:
                msg = await dp.bot.send_message(
        ...
        if call.data == "panel_send_message_accept":
            try:
                ...
                for admin in ADMINS:
                    ...
                    video_send = await dp.bot.send_video(
                        admin,
                        video=video,
                        caption="Відео відправлено",
                        reply_markup=back,
                    )
                    await db.message("BOT", 10001, msg.html_text, msg.date)
                    await db.message(
                        "BOT", 10001, video_send.video.file_id, video_send.date
                    )
                await state.finish()
            except Exception as e:
                msg = await dp.bot.send_message(
...
@dp.callback_query_handler(
    IDFilter(ADMINS),
    lambda call: call.data == "panel_send_message_decline_phone",
    state="send_message_text",
)
@dp.callback_query_handler(
    IDFilter(ADMINS),
    lambda call: call.data == "panel_send_message_accept_phone",
    state="send_message_text",
)
...
@dp.callback_query_handler(
    IDFilter(ADMINS),
    lambda call: call.data == "panel_send_sms_decline",
    state="send_message_text",
)
@dp.callback_query_handler(
    IDFilter(ADMINS),
    lambda call: call.data == "panel_send_sms_accept",
    state="send_message_text",
)
async def message_send_accept_sms(call: types.CallbackQuery, state: FSMContext):
    ...
    if data["type"] == "text":
        ...
        if call.data == "panel_send_sms_accept":
            try:
                msg = await dp.bot.send_message(
                    call.from_user.id, "Починаю відправку SMS..."
                )
                await db.message("BOT", 10001, msg.html_text, msg.date)
                if len(text) >= 53:
                    print("transliterate")
            ...
            msg = await dp.bot.send_message(
                call.from_user.id, "SMS не відправлено", reply_markup=back
            )
            await db.message("BOT", 10001, msg.html_text, msg.date)
            await state.finish()
    else:
        ...
        msg = await dp.bot.send_message(
            call.from_user.id,
            "SMS не відправлено, тільки текстові повідомлення",
            reply_markup=back,
        )
        await db.message("BOT", 10001, msg.html_text, msg.date)
        await state.finish()
...
async def ban_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"Користувач з Telegram ID: {message.text} заблокований", reply_markup=back
        )
    except Exception as e:
...
async def unban_id(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"Користувач з Telegram ID: {message.text} розблокований", reply_markup=back
        )
    except Exception as e:
        await message.answer(f"Помилка: {e}", reply_markup=back)
    await state.finish()
...
async def register_alarm(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.answer()
    await call.message.answer(
        "Виберіть группу аварії, або введіть номери договру котрих стосується аварія.\n"
        "Наприклад:\n"
        "10000001, 10000002, 10000003..",
        reply_markup=grp_choice,
    )
...
async def redacting_alarm_state(call: types.CallbackQuery, state: FSMContext):
    ...
    if records:
        for record in records:
            ...
            keyboard_alarms.add(
                InlineKeyboardButton(f"{record[0]}", callback_data=f"{record[0]}")
            )
...
async def redacting_alarm(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        await call.answer()
        await call.message.edit_text(
            "Напишіть нове повідомлення що стосується аварії", reply_markup=back_inline
        )
...
async def deleting_alarm(call: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await call.message.edit_text(
            f"Аварію з ID {call.data} успішно видалено", reply_markup=back_inline
        )
        await state.finish()
    except Exception as e:
        await call.message.edit_text(f"Помилка: {e}", reply_markup=back_inline)
...
async def message_alarm_redact(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f"Аварія з id: {alarm_id['alarm_id']}\nНовий текст аварії:\n{message.text}",
        reply_markup=back,
    )
    try:
        await call.answer()
...
async def send_broadcast_message(telegram_id: int, data: dict):
    """Відправка одного повідомлення користувачу залежно від типу контенту"""
    content_type = data.get('content_type')

    if content_type == 'text':
        await dp.bot.send_message(telegram_id, data['text'], parse_mode="HTML")
    elif content_type == 'photo':
        await dp.bot.send_photo(telegram_id, data['file_id'], caption=data.get('caption'))
    elif content_type == 'document':
        await dp.bot.send_document(telegram_id, data['file_id'], caption=data.get('caption'))
    elif content_type == 'video':
        await dp.bot.send_video(telegram_id, data['file_id'], caption=data.get('caption'))


@dp.callback_query_handler(IDFilter(ADMINS), text="broadcast_start", state='*')
async def broadcast_start(call: types.CallbackQuery, state: FSMContext):
    """Точка входу - запит контенту для розсилки"""
    await call.answer()
    await state.finish()
    ...
    msg = await call.message.answer(
        ...
        f"Всього користувачів: {user_count}\n\n"
        f"Надішліть текст, фото, документ або відео для розсилки:",
        reply_markup=back
    )
    await db.message(call.from_user.full_name, call.from_user.id, msg.html_text, msg.date)
    await BroadcastStates.WaitingContent.set()


@dp.message_handler(IDFilter(ADMINS), state=BroadcastStates.WaitingContent,
                    content_types=['text', 'photo', 'document', 'video'])
async def broadcast_receive_content(message: types.Message, state: FSMContext):
    """Отримання та збереження контенту, показ preview"""
    await db.message(message.from_user.full_name, message.from_user.id, message.text, message.date)

    # Збереження контенту залежно від типу
    if message.content_type == "text":
        await state.update_data(content_type='text', text=message.text)
        await message.answer("Попередній перегляд повідомлення:")
    ...
    user_count = await db.count_users()
    await message.answer(
        f"Підтвердіть відправку {user_count} користувачам:",
        reply_markup=broadcast_confirm
    )
    await BroadcastStates.PreviewConfirm.set()


@dp.callback_query_handler(IDFilter(ADMINS), text="broadcast_cancel",
                           state=BroadcastStates.PreviewConfirm)
async def broadcast_cancel(call: types.CallbackQuery, state: FSMContext):
    """Скасування розсилки до відправки"""
    await call.answer()
    await state.finish()
    await call.message.answer("Розсилку скасовано", reply_markup=admin_keyboard)


@dp.callback_query_handler(IDFilter(ADMINS), text="broadcast_send",
                           state=BroadcastStates.PreviewConfirm)
async def broadcast_execute(call: types.CallbackQuery, state: FSMContext):
    """Виконання розсилки з прогресом"""
    await call.answer()
    data = await state.get_data()

    # Ініціалізація флагу скасування
    await state.update_data(cancelled=False)
    await BroadcastStates.Broadcasting.set()
    ...
    # Повідомлення прогресу з кнопкою скасування
    progress_msg = await call.message.answer(
        f"Розсилка: 0/{total}...",
        reply_markup=broadcast_stop
    )
    ...
    for idx, user in enumerate(users, 1):
        ...
        if current_data.get('cancelled', False):
            await progress_msg.edit_text(
                f"Розсилку зупинено!\n"
                f"Надіслано: {success}/{total}\n"
                f"Помилок: {failed}",
                reply_markup=None
            )
            await state.finish()
            await call.message.answer("Панель адміністратора", reply_markup=admin_keyboard)
            return

        telegram_id = user['telegram_id']
        try:
            await send_broadcast_message(telegram_id, data)
            success += 1
        except Exception as e:
            failed += 1

        # Оновлення прогресу кожні 10 повідомлень або в кінці
        if idx % 10 == 0 or idx == total:
            try:
                await progress_msg.edit_text(
                    f"Розсилка: {idx}/{total}...\n"
                    f"Успішно: {success}, Помилок: {failed}",
                    reply_markup=broadcast_stop
                )
            except:
                pass  # Ігнорувати "message not modified"

        # Rate limiting: ~25 повідомлень/сек
...
@dp.callback_query_handler(IDFilter(ADMINS), text="broadcast_stop",
                           state=BroadcastStates.Broadcasting)
async def broadcast_stop_handler(call: types.CallbackQuery, state: FSMContext):
    """Встановлення флагу скасування для зупинки розсилки"""
    await call.answer("Зупиняю розсилку...")
