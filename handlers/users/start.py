...
async def call_main_menu(call: CallbackQuery, state: FSMContext):
    ...
    await database.search_query(tel)
    try:
        await db.set_contract(database.data[2], call.from_user.id)
    except IndexError:
        pass
    net_on = __("Увімкнено")
    net_off = __("Вимкнено")
    ...
    if call.from_user.id in ban:
        ...
        if await database.check_net_pause(database.data[2]) is True and database.data[4] == "on":
            if await db.is_alarm(call.from_user.id):
                ...
                await call.message.answer(f"<b>{message_alarm}</b>")
